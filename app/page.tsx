...
export default function HomePage() {
  ...
  // ── Inject comment into Tavus when question changes ───────────────────────
  ...
  // ── Init Typeform widget imperatively ────────────────────────────────────
  useEffect(() => {
    ...
    const init = async () => {
      const { createWidget } = await import('@typeform/embed')
      const widget = createWidget('ENn7cxWK', {
        container: containerRef.current!,
        onQuestionChanged: ({ ref }: { ref: string }) => {
          setCurrentRef(ref)
          injectComment(ref)
        },
        onSubmit: ({ responseId }: { responseId: string }) => {
          ...
          window.location.href = `/results/${responseId}`
        },
        hideFooter: true,
        hideHeaders: true,
        opacity: 0,
      })
      cleanup = () => widget.unmount()
    }
    ...
    const handleMessage = (e: MessageEvent) => {
      try {
        ...
        if (data?.type === 'form-submit' && data?.responseId) {
          window.location.href = `/results/${data.responseId}`
        }
        // Typeform embed SDK v2 format
        if (data?.name === 'form-submit' && data?.data?.responseId) {
          window.location.href = `/results/${data.data.responseId}`
        }
      } catch { /* ignore */ }
    }
